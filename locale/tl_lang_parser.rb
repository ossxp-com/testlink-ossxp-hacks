...
for i in 1..lines_lang_old
    ...
    if(/\*\//.match(lines_lang_old_content[i]))
        ...
        begin_line_old = i + 1
        i=i+1
		puts " * Scripted update according en_GB string file (version: #{revision_comment})"
		puts " * ---------------------------------------------------------------------------------- */"
=begin
    else
...
for i in begin_line..lines_eng
    ...
    elseif /^\TLS_([\w]+)[\s]*=[\s]*(.*)/.match(lines_eng_content[i])
    {
        var_counter+=1;
...
#        print_r(parsed_line);
		...
		for k in begin_line_old..file_lang_old
			if '/^\\'+var_name+'[\s]*=.+/'.match(lines_lang_old_content[k])
		        puts "Found localized variable on (line #{k}) >>> #{lines_lang_old[k]}";
				bLocalized = TRUE;
		        localizedLine = Regexp.last_match[1] 
			    k=k+1	
        ...
        {
            i++;
            puts "(line i) English multiline value - copy the line >>".lines_eng[i];
            orig_eng .= lines_eng[i];
        }
    ...
    {
    	puts "ERROR: please fix the unparsed line (i): " . lines_eng[i];
    	exit;
    }
}
