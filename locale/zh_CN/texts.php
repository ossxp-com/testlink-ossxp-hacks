...
<p> <i>测试规范</i> 允许用户查看和编辑所有现有的" .
		"<i>测试套件</i> 和 <i>测试用例</i>. 同一个测试案例可以保留多个版本.".
		"所有以前的历史版本都将被保留,并且可以在这里进行查看和管理.</p>
...
<li><b>图表</b> - 报告以图表形式显示(flash 技术)</li>
</ul>
...
<h3>通用测试统计报告</h3>
<p>该页只显示测试计划中的测试套件,所有者, 关键字的最新状态.
'当前状态' 是指最近的测试版本的测试执行状态.例如. 一个测试用例在多个构建版本上执行过. 这里只显示最新版本的结果.</p>
...
<ul>
	...
	<li><b>按关键字</b>
	表中列出当前测试计划里所有测试用例里的关键词, 以及对应的测试结果.</li>
	<li><b>按测试者</b>
	列出当前测试计划里分派给各用户的测试用例. 未分配给用户的归类到 'unassigned(未分派)' 栏里.</li>
</ul>

<h3>总体版本状态</h3>
<p>列出各个版本的执行结果. 对于每一个版本, 有总测试用例数, 总通过数, 通过的比例, 总失败数, 失败的比例, 总受阻数, 受阻的比例, 未执行的总数, 未执行的比例. 如果一个测试用例在同一构建版本上执行了多次, 则最近一次的结果才计入统计结果.</p>

<h3>查询统计</h3>
<p>该报表包括一个查询输入表, 一个查询结果页. 查询输入表有四个按钮. 每个按钮的缺省值设置为查询可以包括的最大范围. 用户可以更改按钮以缩小查询范围. 可以按分派人, 关键字, 子类, 版本等组合过滤.</p>

<ul>
<li><b>关键字</b>可以选择 0->1 个关键字. 系统缺省的设置是不选. 如果该关键字不被选中, 则测试规范里和关键字管理页不管有没有分配该关键字的所有测试案例都包括. 一个关键词进入一个测试案例后, 将传播到该测试案例所属于的所有的 测试计划, 以及该案例的所有版本. 如果你只关心有特定的关键词的测试结果, 你要改变控制按钮的值. </li>
