 ...
 * @version $Id: print.inc.php,v 1.69.2.11 2009/06/05 15:24:21 havlat Exp $
 * @uses printDocument.php
 *
 * @internal Library for documents generation
 *
 * Revisions:
 *  20090525 - franciscom - localition for undefined user name + better session name
...
function renderFirstPage($doc_info)
{
	...
	// a page break always separates the 'Title Page' from the rest of the document
...
function gendocGetUserName(&$db, $userId)
{
	...
	else
	{
		...
		else
		{
			...
			// This log on Audit IMHO will only scare user => better say nothing
			// or give a soft message
			tLog("Document Generation - gendocGetUserName() - userId=$userId does not exist on system",'WARNING');
		}
	}
