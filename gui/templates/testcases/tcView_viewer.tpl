...
$Id: tcView_viewer.tpl,v 1.26.2.2 2009/05/25 20:38:14 franciscom Exp $
viewer for test case in test specification

rev:
    20090525 - franciscom - added logic to avoid crash on getDisplayName()
...
{lang_get var="labels"
          s="requirement_spec,Requirements,tcversion_is_inactive_msg,
             ...
             title_last_mod,title_created,by,expected_results,keywords,undefined,
...
{if isset($args_users[$args_testcase.author_id]) }
  {assign var="author_userinfo" value=$args_users[$args_testcase.author_id]}
  {assign var="authorDisplayName" value=$author_userinfo->getDisplayName()}
{else}
  {assign var="authorDisplayName" value=$labels.undefined}
{/if}

{assign var="updaterDisplayName" value=""}
{if $args_testcase.updater_id != ''}

  {if isset($args_users[$args_testcase.updater_id]) }
    {assign var="updater_userinfo" value=$args_users[$args_testcase.updater_id]}
    {assign var="updaterDisplayName" value=$updater_userinfo->getDisplayName()}
  {else}
  ...
  {if $args_show_version == "yes"}
	...
	<tr class="time_stamp_creation">
  		<td width="50%">
      		...
      		{$labels.by}&nbsp;{$authorDisplayName|escape}
  		</td>
    	...
    	<td width="50%">
    		{$labels.title_last_mod}&nbsp;{localize_timestamp ts=$args_testcase.modification_ts}
		  	&nbsp;{$labels.by}&nbsp;{$updaterDisplayName|escape}
    	</td>
  		{/if}
    </tr>
